...
import gain.aura.R
import gain.aura.util.FileUtil.getFileProvider
...
object UpdateUtil {
    ...
    private fun Context.getLatestApk(): File {
        val apkDir = getExternalFilesDir("apk") ?: filesDir.resolve("apk")
        apkDir.mkdirs()
        return File(apkDir, "latest.apk")
    }
        ...
        context.run {
            kotlin
                .runCatching {
                    ...
                    if (!apkFile.canRead()) {
                        throw Exception("APK file is not readable")
                    }
                    
                    // Validate APK is not corrupted by checking if package info can be retrieved
                ...
                .onFailure { throwable: Throwable ->
                    ...
                    Log.e(TAG, "Failed to install APK: ${throwable.message}")
