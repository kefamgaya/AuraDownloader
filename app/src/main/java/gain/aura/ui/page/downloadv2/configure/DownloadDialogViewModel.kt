...
class DownloadDialogViewModel(private val downloader: DownloaderV2) : ViewModel() {
    ...
    private fun proceedWithUrls(action: Action.ProceedWithURLs) {
        ...
        // Fetch video info for the first URL
        ...
        if (isYouTubeUrl(url)) {
            viewModelScope.launch(Dispatchers.Main) {
                mSheetStateFlow.update { 
                    SheetState.Error(
                        action = action, 
                        throwable = IllegalStateException("YouTube videos are not supported")
                    ) 
                }
            }
            return
        ...
        mSheetStateFlow.update { SheetState.Loading(taskKey = "ProceedWithURLs_$url", job = job) }
    }
