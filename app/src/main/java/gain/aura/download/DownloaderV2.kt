...
import gain.aura.R
import gain.aura.download.Task.DownloadState
import gain.aura.download.Task.DownloadState.Canceled
import gain.aura.download.Task.DownloadState.Completed
import gain.aura.download.Task.DownloadState.Error
import gain.aura.download.Task.DownloadState.FetchingInfo
import gain.aura.download.Task.DownloadState.Idle
import gain.aura.download.Task.DownloadState.ReadyWithInfo
import gain.aura.download.Task.DownloadState.Running
import gain.aura.download.Task.RestartableAction.Download
import gain.aura.download.Task.RestartableAction.FetchInfo
import gain.aura.download.Task.TypeInfo
...
class DownloaderV2Impl(private val appContext: Context) : DownloaderV2, KoinComponent {
    ...
    init {
        ...
        scope.launch(Dispatchers.IO) {
            ...
            // Backup all tasks including completed ones, but separate them
            snapshotFlow
                .distinctUntilChanged()
                .collect {
                    it.forEach { Log.d(TAG, it.value.viewState.title) }
                    // Backup active (non-completed) tasks
                    val activeTasks = it.filter { it.value.downloadState !is Completed }
                    PreferenceUtil.encodeTaskListBackup(activeTasks)
                    // Backup completed tasks separately
                    val completedTasks = it.filter { it.value.downloadState is Completed }
                    PreferenceUtil.encodeCompletedTaskListBackup(completedTasks)
                }
        }
    }

    private fun enqueueFromBackup() {
        // Restore active (non-completed) tasks
        val taskList =
        ...
        // Restore completed tasks (don't enqueue them, just add to state map)
        val completedTasks = PreferenceUtil.decodeCompletedTaskListBackup()
        completedTasks.forEach { (task, state) ->
